...
code = code.replace(/import \{ ThreadCanvas \} from '\.\/ThreadCanvas';/, 'import { LivePermissionModal, type LivePermissionRequest } from "./modals";\nimport { ThreadCanvas } from "./ThreadCanvas";');

// 3. Remove LivePermissionRequest interface
code = code.replace(/interface LivePermissionRequest \{\s*readonly sourceEventId: string;\s*readonly runId\?: string;\s*readonly sessionId\?: string;\s*readonly command: string;\s*readonly riskCategory: string;\s*readonly reason: string;\s*readonly evidence: string;\s*readonly requestedAt: Date;\s*\}/, '');

// 4. Remove LivePermissionModal function
code = code.replace(/export function LivePermissionModal\([\s\S]*?<\/section>\s*<\/div>\s*\);\s*\}/, '');
...
for (const capsule of capsules) {
  const regex = new RegExp(`export function ${capsule}\\([\\s\\S]*?<\\/article>\\s*(\\)\\s*)?;?\\s*\\}`);
